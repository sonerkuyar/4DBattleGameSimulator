...
class Grid:
    def __init__(self, num_squares, square_size, padding, offset, fps=10):
        ...
        Args:
            num_squares (tuple): The number of squares in the grid (rows, columns).
            square_size (int): The size of each square in pixels.
            padding (int): The padding between squares in pixels.
            offset (int): The offset from the edge of the canvas in pixels.
            fps (int, optional): The frames per second for animation. Defaults to 10.
        """
        self.row_i = 0
        self.col_i = 0
        self.max_col = 3
    ...
    def initialize_new_window(self, universe_id):
        """
        Initializes a new window for a universe.
    ...
    def update(self, agents, universe_id, t, turn):
        """
        Updates the grid with the positions and status of agents.

        Args:
            agents (dict): A dictionary of agents with their IDs as keys and their attributes as values.
            universe_id (int): The ID of the universe.
